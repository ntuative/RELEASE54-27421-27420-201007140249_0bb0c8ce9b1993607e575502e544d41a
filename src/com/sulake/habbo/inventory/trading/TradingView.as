package com.sulake.habbo.inventory.trading
{
   ...
   public class TradingView implements IInventoryView, IGetImageListener
   {
      ...
      private static const const_785:uint = 4284532064;
      
      private static const const_786:uint = 2577770;
      
      public static const const_1037:int = 0;
      ...
      private var var_361:ItemPopupCtrl;
      
      private var var_24:TradingModel;
      ...
      public function TradingView(param1:TradingModel, param2:IHabboWindowManager, param3:IAssetLibrary, param4:IRoomEngine, param5:IHabboLocalizationManager)
      {
         ...
         var_361 = new ItemPopupCtrl(_loc8_,_assetLibrary);
      }
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            switch(param2.name)
            {
               case "button_accept":
                  switch(var_24.state)
                  {
                     case TradingModel.const_158:
                        if(var_24.otherUserItems.length == 0 && true)
                        {
                           alertPopup(const_1037);
                        }
                        if(false)
                        {
                           var_24.requestUnacceptTrading();
                        }
                        else
                        {
                           var_24.requestAcceptTrading();
                        }
                        break;
                     case TradingModel.const_264:
                        param2.disable();
                        var_24.requestConfirmAcceptTrading();
                  }
                  break;
               case "button_cancel":
                  switch(var_24.state)
                  {
                     case TradingModel.const_158:
                        var_24.requestCancelTrading();
                        break;
                     case TradingModel.const_264:
                        var_24.requestConfirmDeclineTrading();
                  }
            }
         }
      }
      ...
      private function ownThumbEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            var_24.requestRemoveItemFromTrading(param2.id);
         }
         else if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER)
         {
            _loc3_ = var_24.ownUserItems.getWithIndex(param2.id);
            if(!_loc3_)
            {
               return;
            }
            _loc4_ = _loc3_.peek();
            _loc5_ = var_24.getItemImage(_loc4_);
            _loc6_ = "${roomItem.name." + _loc4_.type + "}";
            if(_loc4_.category == FurniCategory.const_168)
            {
               _loc6_ = "${poster_" + _loc4_.stuffData + "_name}";
            }
            if(_loc4_.category == FurniCategory.const_1011)
            {
            ...
            var_361.updateContent(param2 as IWindowContainer,_loc6_,_loc5_,ItemPopupCtrl.const_383);
            var_361.show();
         }
      ...
      public function imageReady(param1:int, param2:BitmapData) : void
      {
         ...
         if(_loc6_)
         {
            updateItemList(var_24.ownUserId);
         }
         _loc5_ = var_24.otherUserItems;
         _loc6_ = false;
         _loc3_ = 0;
         while(_loc3_ < _loc5_.length)
         {
         ...
         if(_loc6_)
         {
            updateItemList(var_24.otherUserId);
         }
      }
      ...
      private function othersThumbEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            _loc3_ = var_24.otherUserItems.getWithIndex(param2.id);
            if(_loc3_ == null)
            {
               return;
            }
      ...
      public function dispose() : void
      {
         if(!_disposed)
         {
            ...
            if(var_179)
            {
               var_179.removeEventListener(TimerEvent.TIMER,timerEventHandler);
               var_179.stop();
               var_179 = null;
            }
            if(var_361 != null)
      ...
      public function updateItemList(param1:int) : void
      {
         ...
         var _loc4_:* = null;
         var _loc2_:int = 0;
         var _loc9_:* = param1 == var_24.otherUserId;
         var _loc10_:Map = !!_loc9_ ? var_24.otherUserItems : var_24.ownUserItems;
      ...
      public function getWindowContainer() : IWindowContainer
      {
         ...
         if(var_1566 == null)
         {
            var_1566 = createMinimizedWindow();
         }
         if(!var_1934)
         {
            return var_67;
      ...
      {
         var _loc2_:* = null;
         if(param1 < var_24.ownUserItems.length)
         {
            _loc2_ = var_24.ownUserItems.getWithIndex(param1) as GroupItem;
            setSelection(_loc2_);
            if(_loc2_)
            {
               var_24.getFurniInventoryModel().displayItemInfo(_loc2_);
            }
         }
      }
      ...
      {
         var _loc2_:* = null;
         if(param1 < var_24.otherUserItems.length)
         {
            _loc2_ = var_24.otherUserItems.getWithIndex(param1) as GroupItem;
            setSelection(_loc2_);
            if(_loc2_)
            {
               var_24.getFurniInventoryModel().displayItemInfo(_loc2_);
      ...
      public function startConfirmCountdown() : void
      {
         if(var_179 == null)
         {
            var_179 = new Timer(1000,3);
            var_179.addEventListener(TimerEvent.TIMER,timerEventHandler);
         }
         var_179.reset();
         var_179.repeatCount = 3;
         var_179.start();
      ...
      public function updateUserInterface() : void
      {
         ...
         if(var_67)
         {
            ...
            if(_loc2_)
            {
               _loc2_.text = var_24.otherUserName;
            }
            _loc1_ = var_67.findChildByTag("OWN_USER_BORDER");
            if(_loc1_)
            {
               _loc1_.color = !true ? uint(const_785) : uint(const_786);
            }
      ...
      public function updateActionState() : void
      {
         ...
         if(var_67)
         {
            ...
            switch(var_24.state)
            {
               case TradingModel.const_180:
                  if(var_24.otherUserItems.length > 0 || var_24.ownUserItems.length > 0)
                  {
                     _loc1_.enable();
                  }
                  else
